...
if __name__ == "__main__":
    ...
    train_set = dataset[0: int((len(dataset) * 0.8))]  # 112
    test_set = dataset[int((len(dataset) * 0.8)):]  # 29

    tree1 = build_tree(train_set)
    train_set_modified = []
    valid_set_mod = []
    for row in train_set:
        row.pop(column_ind)
        train_set_modified.append(row)
    for row in test_set:
        new_row = [row[i] for i in range(0, len(row)) if i != column_ind]
        # for i in range(0, len(row)):
        #     if i != column_ind:
        #         new_row.append(row[i])
        valid_set_mod.append(new_row)
    ...
    for row in test_set:
        klasa_test = row[-1]
        klasa_pred1 = classify(row, tree1)
        for k in klasa_pred1.keys():
            if klasa_test == k:
                acurr1 = acurr1 + 1
    for row in valid_set_mod:
        klasa_test = row[-1]
        klasa_pred2 = classify(row, tree2)
        for k in klasa_pred2.keys():
            if klasa_test == k:
                acurr2 = acurr2 + 1
    print("Tochnost so prvoto drvo na odluka: " + str(acurr1/vk))
    print("Tochnost so vtoroto drvo na odluka: " + str(acurr2/vk))
