...
if __name__ == '__main__':
    ...
    train_set = dataset[: int(np.floor(0.75*len(dataset)))]
    test_set = dataset[int(np.floor(0.75*len(dataset))):]

    docs = [item[0] for item in train_set]
    ...
    tn = 0
    fn = 0
    for comment, cl in test_set:
        vector = process_document(comment, df, N, vocab)
        pred_dec = max(classify(vector, tree).items(), key=operator.itemgetter(1))[0]
        if pred_dec == cl:
            # true
            if pred_dec == "spam":
                tp = tp + 1
    ...
    acurr = (tp + tn) / (tp + tn + fp + fn)
    precision = tp / (tp + fp)
    recall = tp / (tp + fn)

    print("Tochnost:", acurr)
    print("Preciznost:", precision)
    print("Odziv:", recall)
    sample_vector = process_document(test_comment, df, N, vocab)

    prediction_dec_tree = max(classify(sample_vector, tree).items(), key=operator.itemgetter(1))[0]

    print(prediction_dec_tree)