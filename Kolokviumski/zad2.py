...
def get_words_withEmoticons(doc):
    """Поделба на документот на зборови. Стрингот се дели на зборови според
    празните места и интерпукциските знаци
    ...
    words = set()
    for word in re.split('\\W+', doc):
        if 2 < len(word) < 20 and word not in emoticons:
            words.add(word.lower())
    pos_emojy = doc.split(' ')
    for em in pos_emojy:
        if em in emoticons:
...
if __name__ == '__main__':
    ...
    if start_index >= end_index or end_index <= 0 or start_index >= len(data) - 1 or start_index < 0 or end_index > len(data) - 1:
        start_index = 0
        end_index = 3

    train_set = data[:start_index] + data[end_index:]
    test_set = data[start_index:end_index]

    klas1 = NaiveBayes(get_words)
    klas2 = NaiveBayes(get_words_withEmoticons)

    for row in train_set:
        klasa = row[0]
        content = row[1]
        klas1.train(content, klasa)
        klas2.train(content, klasa)
    i = 1
    for val in test_set:
        print(str(i) + ":")
        print(val[1])
        print("Vistinska klasa: " + val[0])
        k1 = klas1.classify_document(val[1])
        k2 = klas2.classify_document(val[1])
        print("Klasa predvidena so Naiven Baes (bez emotikoni): " + k1)
        print("Klasa predvidena so Naiven Baes (so emotikoni): " + k2)
        i = i + 1
