...
if __name__ == '__main__':
    sample_ind = input()
    index = int(sample_ind)
    test_sample = data[index]
    train_set = data[:index] + data[index + 1:]

    docs = [item[1] for item in train_set]
    labels = [item[0] for item in train_set]

    dataset, df, N, vocab = create_dataset(docs, labels)

    tree = build_tree(dataset, entropy)
    sample_vector = process_document(test_sample, df, N, vocab)

    # clas = classify(sample_vector, tree)
    # print(clas)
    # pred = clas.items()
    # max_pred = max(pred, key=lambda a: a[1])
    # print(max_pred)
