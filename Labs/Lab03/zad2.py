...
class NaiveBayes(DocumentClassifier):
    ...
    def calculate_document_probability_in_class(self, item, current_category):
        ...
        p = 1
        for current_feature in features:
            weighted_p, p_word = self.weighted_probability(current_feature, current_category, self.get_feature_per_category_probability)
            p_word_round = round(p_word, 5)
            weighted_p_round = round(weighted_p, 5)
            # print((current_feature, current_category, p_word_round, weighted_p_round))
            self.lab_list.add((current_feature, current_category, p_word_round, weighted_p_round))
            # print(self.lab_list)
            p *= weighted_p
...
if __name__ == "__main__":
    recenica = input()
    # klasifikator = NaiveBayes(get_words_alpha)
    klasifikator = NaiveBayes(get_words)

    for doc in train_data:
        doc_class = doc[1]
        doc_content = doc[0]
        klasifikator.train(doc_content, doc_class)

    klasa = klasifikator.classify_document(recenica)
    prob = klasifikator.get_category_probability_for_document(recenica, klasa)
    log = math.log(prob, 2)
    lab_set = klasifikator.lab_list
    for row in sorted(lab_set):
        print("{} {} {} {}".format(row[0], row[1], row[2], row[3]))
    print("{} {}".format(klasa, round(log, 5)))
