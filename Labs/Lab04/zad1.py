...
def tabela_na_slichni_korisnici(reviews):
    slicnosti = {}
    for person in reviews.keys():
        slicnosti[person] = {}
        for otherPerson in reviews.keys():
            if person != otherPerson:
                euclid = sim_distance(reviews, person, otherPerson)
                pearson = sim_pearson(reviews, person, otherPerson)
                slicnosti[person][otherPerson] = (round(euclid, 3), round(pearson[0], 3), pearson[1])
    return slicnosti
