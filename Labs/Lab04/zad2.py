...
def get_recommendations(prefs, person, similarity=sim_pearson):
    ...
    rankings_custom = rankings
    # Сортирање во растечки редослед по рејтинг
    rankings_custom.sort(key=lambda movie: movie[0])
    # Сортирање на листата во растечки редослед. Превртување на листата за најголемите вредности да бидат први
    rankings.sort(reverse=True)
    return rankings, rankings_custom
...
if __name__ == '__main__':
    ...
    for key, val in ratings.items():
        if key in test_users:
            test_ratings[key] = val
        else:
            train_ratings[key] = val

    for user in test_users:
        tmp_ratings = train_ratings
        tmp_ratings[user] = ratings[user]
        recomendations, custom_rec = get_recommendations(tmp_ratings, user)
        recom = [item for a, item in recomendations]
        if len(recom) > 3:
            recLst = "; ".join(recom[:3])
            print("{}:".format(user), recLst)
        elif len(recom) == 0:
            print("{}:".format(user), "no recommendations ...")
        else:
            recLst = "; ".join(recom)
            print("{}:".format(user), recLst)

        del tmp_ratings[user]
