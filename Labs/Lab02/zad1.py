...
class DecisionNode:
    def __init__(self, col=-1, value=None, results=None, tb=None, fb=None, lvl=None):
        """
...
def build_tree(rows, scoref=entropy, level=0):
    ...
    if best_gain > 0:
        true_branch = build_tree(best_sets[0], scoref, level=level + 1)
        false_branch = build_tree(best_sets[1], scoref, level=level + 1)
        return DecisionNode(col=best_criteria[0], value=best_criteria[1],
                            tb=true_branch, fb=false_branch, lvl=level)
    else:
